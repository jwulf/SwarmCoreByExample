  ...
  getCustomerDetails:{
      ...
      code:function () {
          ...
          getCustomerByEmail(this.email,
            function getCustomerByEmailCallback(err, result) {
              if (err) {
                me.error = err;
                me.swarm("ErrorEndpoint");
              } else {
                me.customer = result;
...
NewCustomerSwarm = {
  ...
  getCustomerDetails:{ # <1>
      ...
      code:function () {
          var me = this;
          getCustomerByEmail(this.email, <3>
            function getCustomerByEmailCallback(err, result) {
              if (err) {
                me.error = err; <4>
                me.swarm("ErrorEndpoint");
              } else {
                me.customer = result; <5>
                me.swarm("Notification");
              }
            });
      }
...
<3> Invokes a function in the adapter running on that node.
<4> Functions execute in the adapter on the node, so a reference to this is created in the closure.
<5> The result
...
The code in the phase code block can invoke functions in the adapter. For example, here the swarm invokes the function _getCustomerByEmail_. This is not a function in the swarm's definition. Rather,it is a function in the CRMAdapter. The swarm invokes the _getCustomerByEmail_ function in the CRMAdapter, and passes as arguments the swarm's _email_ property and a callback function to receive the results.
